...
public class AMapHelp {
	 ...
	 * @annotation 高亮显示查询结果
	 */
	public static void lightShowQueryResult(Drawable drawable, int i,
			String typeName, Geometry geom, GraphicsLayer lightGraphicsLayer) {
		// 高亮显示查询结果
		Random r = new Random();
		Graphic picGraphic = null;
		...
		// 点
		if (typeName.equalsIgnoreCase("point")) {
			// SimpleMarkerSymbol sms = new SimpleMarkerSymbol(R.drawable.icon,
			// 20, STYLE.SQUARE);

			// 文本标识
			TextSymbol textSymbol = new TextSymbol(22.0f, "消防栓", Color.RED);
			textSymbol.setOffsetX(-20);
			textSymbol.setOffsetY(-30);
			textGraphic = new Graphic(geom, textSymbol);

			// 图片标识
			PictureMarkerSymbol pictureMarkerSymbol = new PictureMarkerSymbol(
					drawable);
			picGraphic = new Graphic(geom, pictureMarkerSymbol);

		}
		// 线
		else if (typeName.equalsIgnoreCase("polyline")) {
			SimpleLineSymbol sls = new SimpleLineSymbol(color, 5);
			picGraphic = new Graphic(geom, sls);
		}
		// 面
		else if (typeName.equalsIgnoreCase("polygon")) {
			SimpleFillSymbol sfs = new SimpleFillSymbol(color);
			sfs.setAlpha(75);
			picGraphic = new Graphic(geom, sfs);
		}

		lightGraphicsLayer.addGraphic(picGraphic);
		lightGraphicsLayer.addGraphic(textGraphic);
	}
